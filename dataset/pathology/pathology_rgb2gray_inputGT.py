...
from mindspore.dataset import  Dataset, GeneratorDataset ,ConcatDataset , RandomSampler,WeightedRandomSampler
...
class DataLoaderX(GeneratorDataset):
    def __iter__(self):
...
norm_std=np.array([[[0.229, 0.224, 0.225, 1]]])
...
class PathologyDataset(Dataset):
    def __init__(self, image_root, label_root, name_list=None, if_training=False, crop=None, resz=None, stain_norm=False, division=32, \
        ...
        for idx, name in enumerate(qbar):
            ...
            gimage = skimage.color.rgb2gray(image).reshape(image.shape[0], image.shape[1], 1)
            if image.max() > 1 and gimage.max() <= 1:
                gimage = np.float32(gimage)
            image = np.float32(image) / 255.0
            image = np.concatenate((image, gimage), axis=-1)
        ...
        print(self.other_domain_root, len(self.other_domain_list))
    ...
    def __getitem__(self, idx):
        ...
        if self.crop is not None:
            ...
            if self.if_training and self.aug:
                if h>self.crop and w>self.crop:
                    if (seg>0).any():
                        hids, wids = np.where(np.max(seg, axis=-1)>0)
                        min_hids = np.min(hids)
                        max_hids = np.max(hids)
                        min_wids = np.min(wids)
                        max_wids = np.max(wids)
                        sh_0 = max(0, min_hids-self.crop//2)
                        sh_1 = min(h-self.crop, max_hids+self.crop//2)
                        sh_0 = min(sh_0, sh_1)
                        sh = random.randint(sh_0, sh_1)
                        sw_0 = max(0, min_wids-self.crop//2)
                        sw_1 = min(w-self.crop, max_wids+self.crop//2)
                        sw_0 = min(sw_0, sw_1)
                        sw = random.randint(sw_0, sw_1)
                    else:
        ...
        if self.size_aug:
            ...
            if p_select>=0.667:
                ...
                itmp_union = np.maximum(itmp_seg[:,:,0], itmp_seg[:,:,1])
                itmp_intersection = np.minimum(itmp_seg[:,:,0], itmp_seg[:,:,1])
        ...
        if self.aug and random.random()>=0.5:
            image = gaussian(image, sigma=(random.random(), random.random()), channel_axis=3)
        ...
        # Freq
        if self.if_training and self.aug and self.freq_aug == 'v0' and random.random()>=0.5:
            p = random.random()
            if self.other_domain_root is not None:
                if random.random() >= 0.5:
                    aug_img_path = self.other_domain_list[random.randint(0, len(self.other_domain_list)-1)]
                    aug_image = skimage.io.imread(aug_img_path)
                    if len(aug_image.shape) == 2:
                        aug_image = skimage.color.gray2rgb(aug_image)
                    aug_image = np.float32(aug_image[:, :, 0:3]) / 255.0
                else:
                    aug_image = self.images[random.randint(0, len(self.images)-1)][:,:,:3].copy()
            else:
                aug_image = self.images[random.randint(0, len(self.images)-1)][:,:,:3].copy()
            aug_image = cv2.resize(aug_image, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST) 
            aug_image = freq_swap_aug(image[:,:,:3], aug_image, L=self.bnd_width_freq_aug, ratio=p)
            aug_image = np.reshape(aug_image, (-1, 3))
            ref_ = np.argsort(np.argsort(np.reshape(aug_image, (-1, 3)), axis=0), axis=0)
            values_ = np.sort(aug_image, axis=0)
            ori_image_shape = image[:,:,:3].shape
            aug_image = np.take_along_axis(values_, ref_, axis=0)
            # aug_image = aug_image.reshape(ori_image_shape)
            image = np.concatenate((aug_image.reshape(ori_image_shape), image[:, :, 3:]), axis=-1)
        ...
        # aug_image = (aug_image - norm_mean) / norm_std
        ...
        # Random FG Suppression
        if self.if_training and self.aug and random.random() >= 0.5:
            sup_ratio = random.random()*0.5
            fg_mask = np.max(segmsk, axis=0, keepdims=True)
            if fg_mask.sum()>=30:
                rev_mask = np.clip(gaussian(fg_mask * sup_ratio, sigma=(0.1+0.4*random.random(), 0.1+0.4*random.random()), channel_axis=0), a_min=0, a_max=1)
                fg_mean = ((image * fg_mask).sum(axis=(-2, -1), keepdims=True) + 1e-7) / (fg_mask.sum(axis=(-2, -1), keepdims=True) + 1e-7)
                bg_mean = ((image * (1-fg_mask)).sum(axis=(-2, -1), keepdims=True) + 1e-7) / ((1-fg_mask).sum(axis=(-2, -1), keepdims=True) + 1e-7)
                fg_rev_image = image - fg_mean + bg_mean
                image = image * (1-rev_mask) + fg_rev_image * rev_mask

        if self.return_dist:
            dist = np.transpose(star_dist(inst_label, self.n_dist).astype(np.float32), (2,0,1))

        outputs = [np.concatenate((image, segmsk), axis=0), segmsk]
...
def getTrainDataLoaders(image_root, label_root, batch_size, name_list=None, \
        ...
        gamma_aug=True, stain_aug=False, stain_mix=False, stain_norm=False, size_aug=False, with_canny=False, freq_aug=None, other_domain_root=None, \
        rnd_pad=False, use_prefetch=False, max_rnd_resz=1.0, **kwargs):
    ...
    if other_domain_root is None:
        other_domain_root = []
    if not isinstance(other_domain_root, (list, tuple)):
        other_domain_root = [other_domain_root, ]

    if isinstance(image_root, (list, tuple)):
        trainset = []
        ori_other_domain_root = other_domain_root
        for i_domain, (i_image_root, i_label_root, i_name_list_0) in enumerate(zip(image_root, label_root, name_list[0])):
            other_domain_root = [image_root[i_other] for i_other in np.setdiff1d(list_all, i_domain).tolist()] + ori_other_domain_root
        ...
        trainset = PathologyDataset(image_root, label_root, name_list=name_list[0], if_training=True, crop=train_crop, resz=resz, \
            ...
            return_domain_lbl=return_domain_lbl, return_domain_confidence=return_domain_confidence, return_tgt_image=return_tgt_image, return_contour=return_contour, return_he=return_he,\
            aug=aug, gamma_aug=gamma_aug, stain_aug=stain_aug, stain_mix=stain_mix, stain_norm=stain_norm, size_aug=size_aug, with_canny=with_canny, freq_aug=freq_aug, max_rnd_resz=max_rnd_resz, other_domain_root=other_domain_root, **kwargs)
    ...
    if use_prefetch:
        ...
        trainloader = GeneratorDataset(trainset, batch_size=batch_size, shuffle=shuffle, sampler=sampler, num_workers=1, drop_last=True)
...
def getSeparateTrainDataLoaders(image_root, label_root, batch_size, name_list=None, \
    ...
    for i_domain, (i_image_root, i_label_root, i_name_list_0) in enumerate(zip(image_root, label_root, name_list[0])):
        ...
        trainloader.append(GeneratorDataset(trainset, batch_size=batch_size, shuffle=False, sampler=sampler, num_workers=min(max(batch_size, 1), 2), drop_last=True))
    return trainloader
...
class PathologyTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gimage = skimage.color.rgb2gray(ori_img).reshape(ori_img.shape[0], ori_img.shape[1], 1)
        if ori_img.max() > 1 and gimage.max() <= 1:
            gimage = np.float32(gimage)
...
def getTestDataLoaders(image_root, label_root, batch_size, name_list=None, \
    ...
    testloader = GeneratorDataset(testset, batch_size=batch_size, shuffle=shuffle, sampler=None, num_workers=num_workers, collate_fn=collate_return_list)
