...
class DomainClsLoss(nn.Module):
    ...
    def forward(self, predictions, targets, **kwargs):
        ...
        acc = F.mean((F.argmax(p, dim=1).view(-1) == targets[self.domain_tgt_loc].view(-1)).float())
        return loss, {'cls_loss': loss, 'acc': acc, 'n1':F.mean((targets[self.domain_tgt_loc]==1).float())}
...
class SegLossDSCRatio(nn.Module):
    ...
    def forward(self, predictions, targets, **kwargs):
        ...
        segloss /= n_batch
        segdiceloss /= n_batch
        ratio_target = F.mean(seg_target, dim=(-2, -1), keepdim=True)
