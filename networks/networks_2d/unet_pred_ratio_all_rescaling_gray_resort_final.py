...
def resort(x1, x1_norm):
    b,c,h,w = x1.shape
    ref = F.argsort(F.argsort(x1.view(b,c,-1), dim=-1), dim=-1)
    val = F.sort(x1_norm.view(b,c,-1),dim=-1)[0]
    x1 = F.gather_elements(val, -1, ref)
    x1 = x1.view(b,c,h,w)
    return x1   
...
class up(nn.Module):
    def __init__(self, in_ch_1, in_ch_2, out_ch, bilinear=True, norm=IN_MLP_Rescaling):
        ...
        if bilinear:
            ...
            self.up = nn.Conv2dTranspose(in_ch_1, in_ch_2, 2, stride=2)
...
class Scaling(nn.Module):
    def __init__(self, num_features, biasic_rate=1):
        ...
        self.scale = nn.Parameter(F.ones([1, num_features,1,1])*biasic_rate, requires_grad=True)
    def forward(self, x):
        return self.scale*x
...
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_features=32, n_out=2, norm=IN_MLP_Rescaling, sigmoid_prob=True, rev_aug=False, biasic_rate=1, with_gray=True):
        ...
        if self.with_gray:
            self.inc_gray2rgb = nn.SequentialCell(
        ...
        self.ratio_feat_inc = nn.Dense(n_features, n_features)
        self.ratio_feat_down1 = nn.Dense(n_features, n_features*2)
        self.ratio_feat_down2 = nn.Dense(n_features, n_features*4)
        self.ratio_feat_down3 = nn.Dense(n_features, n_features*8)
        self.ratio_feat_down4 = nn.Dense(n_features, n_features*8)
        ...
        for m in self.modules():
            ...
            if (valid_w or valid_b) and (isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv2dTranspose) or isinstance(m, nn.Dense)):
                if valid_w:
                    # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    mindspore.common.initializer.HeUniform(m.weight, mode='fan_out', nonlinearity='relu')
                if valid_b and m.bias is not None:
                    # nn.init.normal_(m.bias, 0.0, 1.0)
                    mindspore.common.initializer.Normal(m.bias, 1.0, 0.0)
                ...
                if valid_w and m.weight is not None:
                    ...
                    mindspore.common.initializer.constant(m.weight, 1)
                if valid_b and m.bias is not None:
                    # nn.init.constant_(m.bias, 0)
                    mindspore.common.initializer.constant(m.weight, 0)

    def forward(self, img, sigmoid_prob=True, prior_ratio=None):
        bsz = img.shape[0]
        
        if img.shape[1] > 4:
            img, gt = F.split(img, (4, self.n_out), dim=1)
            gt_ratio = gt.mean(dim=(-2,-1)).view(gt.shape[0], self.n_out, 1, 1)
        else:
            gt = None
            gt_ratio = None
        if F.isnan(img).any() or F.isinf(img).any():
            assert(False)

        if self.with_gray:
            img, img_gray = F.split(img, (3, 1), dim=1)
            if self.training and self.rev_aug:
                img_gray_rev = -img_gray
                mask = (F.rand(bsz, 1, 1, 1)>=0.5).float().to(img_gray_rev.device)
        ...
        if gt_ratio is not None and self.training:
            ...
            cur_feat_ratio = F.cat((pred_feat_ratio, gt_feat_ratio), dim=0)
        else:
            if prior_ratio is None:
        ...
        if self.with_gray:
            ...
            if self.training:
                img_r = F.cat((img, img_gray - img_gray.detach() + resort(img, img_gray)), dim=0)
...
if __name__=='__main__':
    ...
    st = time.time()
    for _ in range(100):
        # model.forward(F.rand([1,3,224,224]).cuda())
        model.forward(F.rand([1,3,224,224]))
    et = time.time()
    print((et-st)/100)
