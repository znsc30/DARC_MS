...
def load_model(model, Model_name, Train_mode, Dataset):
    ...
    if os.path.isfile(filepath):
            ...
            model.load_state_dict(mindspore.load(filepath))
            return model
    else:
...
def save_model(model,trainAcc_to_file,testAcc_to_file,trainloss_to_file,testloss_to_file,Parameters,
               Model_name,Train_mode,Dataset,model2=None,remove_checkpoints=True,**kwargs):
        try:
            model=model.module
        except:
            pass
        ...
        if model2 is not None:
            ...
            mindspore.save_checkpoint(model.state_dict(),path+weights_filename1)
            mindspore.save_checkpoint(model2.state_dict(),path+weights_filename2)
        else:
            weights_filename=Model_name+'_'+Train_mode+'_'+Dataset+'.t7'
            mindspore.save_checkpoint(model.state_dict(),path+weights_filename)
        ...
        if remove_checkpoints:
            ...
            shutil.move(os.getcwd()+'/CHECKPOINT/checkpoint_'+Model_name+'_'+Train_mode+'_'+Dataset, path)

  
def checkpoint_save_stage(model,trainloss_to_file,testloss_to_file,train_metric_to_file,test_metric_to_file,Parameters,Model_name,Train_mode,Dataset,model2=None, checkpoint_suffix=None):
        ...
        if checkpoint_suffix is not None:
            if model2 is not None:
                mindspore.save_checkpoint(model.state_dict(),path+'/CHECKPOINT1'+checkpoint_suffix+'.t7')
                mindspore.save_checkpoint(model2.state_dict(),path+'/CHECKPOINT2'+checkpoint_suffix+'.t7')
            else:
                mindspore.save_checkpoint(model.state_dict(),path+'/CHECKPOINT'+checkpoint_suffix+'.t7')
            print(path+'/CHECKPOINT'+checkpoint_suffix+'.t7'+' saved')  

        if model2 is not None:
            mindspore.save_checkpoint(model.state_dict(),path+'/CHECKPOINT1.t7')
            mindspore.save_checkpoint(model2.state_dict(),path+'/CHECKPOINT2.t7')
        else:
            mindspore.save_checkpoint(model.state_dict(),path+'/CHECKPOINT.t7')
