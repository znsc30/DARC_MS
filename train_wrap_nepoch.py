...
from mindspore.experimental.optim.lr_scheduler import ReduceLROnPlateau,StepLR
    ...
    def Train(self, model, optimizer, TrainSet, TestSet, Train_mode, Model_name, Dataset, epochs=None, scheduler=None, fix_amp_epoch=None):
        ...
        if isinstance(scheduler, ReduceLROnPlateau):
            ...
            if scheduler_mode == 'min':
                scheduler.step(tensor([math.inf],mindspore.float16))
            else:
                scheduler.step(tensor([-math.inf],mindspore.float16))
        ...
        while keep_training:
            ...
            if self.valid_in_train:
                if epoch % self.validate_every == 0 :
                    ...
                    if isinstance(scheduler, ReduceLROnPlateau):
                        ...
                        if scheduler.num_bad_epochs==0 and prev_num_bad_epochs==scheduler.patience and not save:
                            ...
                            model.load_state_dict(mindspore.load(path_checkpoint))
                ...
                if epoch%self.save_every==0 or epoch == epochs:
                    mindspore.save_checkpoint(model.state_dict(), os.getcwd()+'/CHECKPOINT/checkpoint_'+Model_name+'_'+Train_mode+'_'+Dataset+'/CHECKPOINT_'+'epoch_'+str(epoch)+'.t7')
    ...
    def train_scratch(self, model, DataSet, optimizer, epoch=None): #eval is not correct in the method
        ...
        for batch_idx, data in enumerate(pbar):
            ...
            if self.with_canny:
                ...
                inputs = F.split(inputs, split_size_or_sections=3, dim=1)
                n_split = len(inputs)
                inputs = F.cat(inputs, dim=0)
                targets = [ F.cat([target, ]*n_split, dim=0) for target in targets ]
            ...
            if self.randconv is not None:
                ...
                if self.restain:
                    ...
                    sorted_rep, _ = F.sort(inputs.view(ori_shape[0], ori_shape[1], -1), dim=-1)
                    for i_aug, aug_inputs in enumerate(inputs_new[1:]):
                        _, argsort_x_idx = F.sort(aug_inputs.view(ori_shape[0], ori_shape[1], -1), dim=-1)
                        inverse_argsort_x_idx = argsort_x_idx.argsort(-1)
                        inputs_new[1+i_aug] = F.gather(sorted_rep, -1, inverse_argsort_x_idx).view(ori_shape)
