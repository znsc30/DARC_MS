            ...
            for isplit, train_dataset_name in enumerate(DATASET_NAME):
                ...
                for cur_domain_idx, (image_root, label_root) in enumerate(zip(All_Test_Split_IMG, All_Test_Split_MSK)):
                    ...
                    for ickp in [100, ]:
                        ...
                        if load_ema:
                            weights = ms.load(os.path.join('./CHECKPOINT', 'checkpoint_'+model_name+'_'+train_mode+'_'+dataset, 'CHECKPOINT_epoch_'+str(ickp)+'.t7'))
                            ...
                            load_info = model.load_state_dict(\
                                ms.load(os.path.join('./CHECKPOINT', 'checkpoint_'+model_name+'_'+train_mode+'_'+dataset, 'CHECKPOINT_epoch_'+str(ickp)+'.t7')),\
                                strict = True
                            )
                            print(load_info)
                        model.eval()
                        ...
                        for i, batch in qbar:
                            ...
                            image_input = tensor(image,ms.float16)
                            ...
                            predictions = F.cat(predictions, dim=0).view(image_patches_shape[0], image_patches_shape[2], image_patches_shape[3], 2, valid_size, valid_size)
                            predictions = F.permute(predictions, (0,3,1,4,2,5)).contiguous().view(b,2,valid_size*image_patches_shape[2],valid_size*image_patches_shape[3])
